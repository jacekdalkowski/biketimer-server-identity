...
def kill_and_remove_all_containers(ssh, container_name):
    # docker ps -a -q
    running_containers = ssh('docker ps -q -f "name=' + container_name + '"')
    ...
    all_containers = ssh('docker ps -a -q -f "name=' + container_name + '"')
    if all_containers:
